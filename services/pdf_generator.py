...
class PDFGenerator:
    ...
    def _create_executive_summary(self, lead_data, enriched_data):
        ...
        summary_text = f"""
        This personalized business audit has been prepared for <b>{company_name}</b>, a {industry.lower()} company.
        Based on our analysis of your company's profile and industry positioning, we have identified key opportunities
        for operational improvement and strategic growth. This report provides actionable insights tailored to
        your company's specific context and the current market landscape.
        """
    ...
    def _create_company_profile(self, lead_data, enriched_data):
        ...
        if enriched_data.get('linkedin_url') or enriched_data.get('twitter_url'):
            ...
            elements.append(Paragraph("Social Presence", self.subheading_style))

            social_links = []
            if enriched_data.get('linkedin_url'):
                social_links.append(f"LinkedIn: {enriched_data['linkedin_url']}")
            if enriched_data.get('twitter_url'):
                social_links.append(f"Twitter: {enriched_data['twitter_url']}")

            for link in social_links:
                elements.append(Paragraph(link, self.caption_style))

        return elements

    def _create_market_insights(self, industry):
        from services.enrichment import get_industry_insights
    ...
    def _create_next_steps(self, lead_data):
        ...
        next_steps_text = """
        We believe these insights provide a strong foundation for your business growth. To discuss
        these findings in detail or explore how we can support your implementation, we invite you
        to schedule a consultation. Our team is ready to help you prioritize and execute these
        recommendations based on your specific timeline and resources.
        """
        ...
        cta_text = """
        <b>Ready to take the next step?</b> Contact us to schedule a personalized strategy session.
        """
    ...
    def _create_footer(self):
        ...
        disclaimer = """
        <i>Disclaimer: This report has been automatically generated based on publicly available information
        and industry trends. The insights provided are for informational purposes only and should not be
        considered as professional business advice. Data accuracy depends on source availability.</i>
        """
        ...
        footer_text = Paragraph(
            "Generated by LeadFlow Pro | Automated Business Intelligence",
            self.caption_style
        )
